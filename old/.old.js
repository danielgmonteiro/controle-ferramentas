...
document.addEventListener('DOMContentLoaded', () => {
  ...
  aplicarTema();
  atualizarIdioma();
  loadGithubConfig(); // Carregar configurações do GitHub
  mostrarSecao('funcionarios');
  
  // Adicionar botão de carregar dados
  const syncButton = document.querySelector('.sync-button');
  syncButton.insertAdjacentHTML('afterend', 
    '<button class="sync-button" onclick="loadFromGitHub()">⬇️ Carregar Dados</button>'
  );
});
...
async function checkFileExists() {
  try {
    ...
    if (!response.ok) throw new Error('Erro ao verificar arquivo');
